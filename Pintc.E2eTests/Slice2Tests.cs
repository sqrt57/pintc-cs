...
public class Slice2Tests : IDisposable
{
    readonly string _tempDir = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());

    public Slice2Tests() => Directory.CreateDirectory(_tempDir);
    public void Dispose() => Directory.Delete(_tempDir, recursive: true);

    [Fact]
    public void Compiles_and_exits_with_0()
    {
        var sourcePath = Path.Combine(_tempDir, "slice2.pnt");
        var exePath    = Path.Combine(_tempDir, "slice2.exe");
        File.WriteAllText(sourcePath, SliceFixtures.Slice2Source);
