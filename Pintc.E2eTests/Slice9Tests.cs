...
public class Slice9Tests : IDisposable
{
    readonly string _tempDir = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());

    public Slice9Tests() => Directory.CreateDirectory(_tempDir);
    public void Dispose() => Directory.Delete(_tempDir, recursive: true);

    [Fact]
    public void Compiles_and_exits_with_0()
    {
        var sourcePath = Path.Combine(_tempDir, "slice9.pnt");
        var exePath    = Path.Combine(_tempDir, "slice9.exe");
        File.WriteAllText(sourcePath, SliceFixtures.Slice9Source);
