...
public class Slice6Tests : IDisposable
{
    readonly string _tempDir = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());

    public Slice6Tests() => Directory.CreateDirectory(_tempDir);
    public void Dispose() => Directory.Delete(_tempDir, recursive: true);

    [Fact]
    public void Compiles_and_exits_with_0()
    {
        var sourcePath = Path.Combine(_tempDir, "slice6.pnt");
        var exePath    = Path.Combine(_tempDir, "slice6.exe");
        File.WriteAllText(sourcePath, SliceFixtures.Slice6Source);
